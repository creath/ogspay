...
uint256 hashGenesisBlock("0x000001120c626312c395fa64dda3ae34ba6d14fb82c3968edcdc464f6aa01b41");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OGSPay: starting difficulty is 1 / 2^12
static CBigNum bnInitialDifficulty(~uint256(0) >> 23);
...
const string strMessageMagic = "OGSPay Signed Message:\n";
...
{
    // OGSPay: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // OGSPay: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "ogspay genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc8, 0xb9, 0xde }; // OGSPay: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OGSPayMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OGSPayMiner : ProcessBlock, block not accepted");
    }
